(function (root, factory) {
        ...
        fields.forEach(function (f) {
            ...
            (f.options || []).forEach(function (opt) {
                ...
                kids.forEach(function (c) {
                    ...
                    if (f.type === 'select') layoutItems.push({ name: c.key, grid: 12 });
                });
            });
        });
    ...
    function importConfig(doc) {
        ...
        (uiSchema.layout || []).forEach(function (row) {
            (row.fields || []).forEach(function (f) {
                ...
                if (orderNames.indexOf(name) === -1) orderNames.push(name);
            });
        });

        var hiddenSet = {};
        Object.keys(props).forEach(function (name) {
            if (gridMap[name] !== undefined || orderNames.indexOf(name) !== -1) return;
        ...
        orderNames.forEach(function (name) {
            ...
            rules.forEach(function (r) {
                ...
                r.children.forEach(function (ck) {
                    ...
                    opt.children.push(fieldFromProp(ck, cp, ctype, true, 12));
                });
            });
            fields.push(f);
        });
    ...
    function allTypes() {
        return ['text', 'number', 'email', 'url', 'phone', 'textarea', 'date', 'select', 'radio', 'checkbox', 'hidden'];
    }
