...
- API công khai:
  - `formEl.schema` / `formEl.uiSchema` — setter, tự render lại.
  - `formEl.data = {…}` — prefill bản ghi khi sửa (gán 1 lần khi mở form; override `default`).
  - `formEl.submitForm()` — validate + emit từ nút ngoài form.
  - `formEl.setLists({ field: [ { value, label } ] })` — gán options từ API; ưu tiên hơn `oneOf` tĩnh; gọi lại nhiều lần được ([4.5](#45-options-nạp-từ-api-setlists-và-optionkeys)).
  - Event `onFormSubmit` — phát khi validate thành công, `e.detail` là dữ liệu form.
